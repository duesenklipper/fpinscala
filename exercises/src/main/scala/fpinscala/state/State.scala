...
object RNG {
  ...
  def double(rng: RNG): (Double, RNG) = {
    ...
    if (i1 < i2) (i1.toDouble / i2, r2) else (i2.toDouble / i1, r2)
  }
  ...
  def ints(count: Int)(rng: RNG): (List[Int], RNG) = if (count <= 0) (Nil, rng) else {
    val (i, r) = rng.nextInt
...
object State {
  ...
  def sequence[S, A](fs: List[State[S, A]]): State[S, List[A]] =
    fs.foldRight(unit[S,List[A]](Nil: List[A]))((stateA, acc) => stateA.map2(acc)(_ :: _))
